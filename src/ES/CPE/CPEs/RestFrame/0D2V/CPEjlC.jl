...
"""
  ...
  Outputs:
    CPEjlC!(out, x, L, nMod;nh=nh,uh=uh,vhth=vhth,vhth2=vhth2,uvth2=uvth2,Mhst=Mhst,
...
# nMode = 2
function CPEjlC!(out::AbstractVector{T}, x::AbstractVector{T}, L::Int;
    ...
    elseif L == 1
        CPEj1C!(out, x;nh=nh,uh=uh,vhth=vhth,vhth2=vhth2,uvth2=uvth2,Mhst=Mhst)
    else 
        # vec = 1:nMod - 1
        vec = 1
        nh[vec] = x[1]
        ...
        uhr2 = ((L+2.5) * Mh2^2 - (L+1.5) * Mh4)^0.5
        ...
        if abs(Mhr0) ≤ rtol_Mh
            if abs(uh[nMod]) ≤ rtol_Mh
                ...
                if iseven(L)
                    nh[nMod] = Mhr0 / uhr2 ^(L/2)
                else
                    nh[nMod] = sign(Mhr0) * Mhr0 / uhr2 ^(L/2)
                end
            end
        else
            if abs(uh[nMod]) ≤ rtol_Mh
        ...
        vhth[nMod] = (1/(L+1.5) * Mh2 - uhr2)^0.5
...
# nMode ≥ 3
function CPEjlC!(out::AbstractVector{T}, x::AbstractVector{T}, L::Int, nMod::Int;
    nh::AbstractVector{T}=[0.1, 1.0], uh::AbstractVector{T}=[0.1, 1.0], 
    vhth::AbstractVector{T}=[0.1, 1.0], vhth2::AbstractVector{T}=[0.1, 1.0], 
    uvth2::AbstractVector{T}=[0.1, 1.0], Mhst::AbstractVector{T}=[0.1, 1.0],
    rtol_OrjL::T=1e-10,atol_Mh::T=1e-10,rtol_Mh::T=1e-10) where{T}
    ...
    elseif L == 1
        ...
        j = L + 4
        N = 2
        OrjL[vec] = OrjLNb(uvth2[vec],j,L,N;rtol_OrjL=rtol_OrjL)
        Mr4 = sum((nh[vec] .* vhth2[vec].^N .* uhL[vec]) .* (1 .+ OrjL[vec]))
        Mh4 = Mhst[nj] - Mr4
    
        uhr2 = ((L+2.5) * (Mh2) - (L+1.5) * Mh4)^0.5
...
"""
  ...
  Outputs:
    JacobCPEjlC!(JM, x, L, nMod;nh=nh,uh=uh,vhth=vhth,vhth2=vhth2,uvth2=uvth2,
...
# nMode ≥ 3
function JacobCPEjlC!(JM::AbstractArray{T,N2}, x::AbstractVector{T}, L::Int, nMod::Int; 
    nh::AbstractVector{T}=[0.1, 1.0], uh::AbstractVector{T}=[0.1, 1.0], 
    vhth::AbstractVector{T}=[0.1, 1.0], vhth2::AbstractVector{T}=[0.1, 1.0], 
    ...
    if L == 0
        JacobCPEj0C!(J, x, nMod;nh=nh,uh=uh,vhth=vhth,vhth2=vhth2,uvth2=uvth2,
                DMjL=DMjL,J=J,DMrjL=DMrjL,VnrL=VnrL,M1jL=M1jL,
                vth1jL=vth1jL,O1jL2=O1jL2,O1jL=O1jL,OrnL2=OrnL2,OrnL=OrnL)
    elseif L == 1
        JacobCPEj1C!(J, x, nMod;nh=nh,uh=uh,vhth=vhth,vhth2=vhth2,uvth2=uvth2,
                DMjL=DMjL,J=J,DMrjL=DMrjL,VnrL=VnrL,M1jL=M1jL,
                vth1jL=vth1jL,O1jL2=O1jL2,O1jL=O1jL,OrnL2=OrnL2,OrnL=OrnL)
    else
        fill!(JM, 0.0)
        ...
        for s in vec
            ...
            for k in vec
                for i in 1:3
                    ...
                    ddcMhjlC0D2V!(DMjL,J,DMrjL,nh[s],uh[s],vhth[s],
                           uh[nMod],vhth[nMod],uaiL,vthijL,ua1L,vth1jL[nj],VnrL,
                           M1jL,O1jL2[nj],O1jL[nj],OrjL2,OrjL,OrnL2,OrnL,j,L;atol_Mh=atol_Mh,rtol_Mh=rtol_Mh)
                    JM[nj, sn1] = DMjL[1]
                    JM[nj, sn2] = DMjL[2]
                    JM[nj, sn3] = DMjL[3]
                end
        ...
        #             OrjL2,OrjL = OrjLN2Nb(uvth2[s],j,L,N;rtol_OrjL=rtol_OrjL)
        #             ddcMhjlC0D2V!(DMjL,J,DMrjL,nh[s],uh[s],vhth[s],
        #                    uh[nMod],vhth[nMod],uaiL,vthijL,ua1L,vth1jL,

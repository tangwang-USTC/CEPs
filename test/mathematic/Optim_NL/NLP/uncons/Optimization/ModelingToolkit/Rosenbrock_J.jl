...
x0 = zeros(2) 
rosenbrock(x, p = nothing;pp=[1,100]) = (pp[1] - x[1])^2 + pp[2] * (x[2] - x[1]^2)^2

function rosenbrock2(x,p;pp=[1,100])
    
    a = (pp[1] - x[1])^2 + pp[2] * (x[2] - x[1]^2)^2
    return a
end
...
@testset "No AD" begin
    optf = OptimizationFunction(rosenbrock2; grad = g!, hess = h!)

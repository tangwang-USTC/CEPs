...
function SimpleNLSModel(::Type{T}) where {T}
  meta = NLPModelMeta(
    2,                                        # number of bsiables
    x0 = ones(T, 2),                          # initial guess
    name = "Simple NLS Model",
    lbs = zeros(T, 2),                       # vector of lower bounds
    ubs = ones(T, 2),                        # vector of upper bounds
    ncon = 3,                                 # total number of general constraints
    lcon = [zero(T); zero(T); one(T)],        # vector of constraint lower bounds
    ucon = [T(Inf); T(Inf); one(T)],          # vector of constraint upper bounds
    nnzj = 6,                                 # number of nonzeros in the sparse Jacobian
  )
